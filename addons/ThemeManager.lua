...
do
    ...
    -- Split version: Colors on left, Presets on right
    function ThemeManager:ApplyToTabSplit(tab)
        ...
        -- Colors section
        colorsBox:AddLabel("Background color"):AddColorPicker("BackgroundColor", { Default = self.Library.Scheme.BackgroundColor })
        colorsBox:AddLabel("Main color")      :AddColorPicker("MainColor",       { Default = self.Library.Scheme.MainColor })
        colorsBox:AddLabel("Accent color")    :AddColorPicker("AccentColor",     { Default = self.Library.Scheme.AccentColor })
        colorsBox:AddLabel("Outline color")   :AddColorPicker("OutlineColor",    { Default = self.Library.Scheme.OutlineColor })
        colorsBox:AddLabel("Font color")      :AddColorPicker("FontColor",       { Default = self.Library.Scheme.FontColor })
        ...
        presetsBox:AddButton("Create theme", function()
            local name = self.Library.Options.ThemeManager_CustomThemeName.Value
            if name:gsub(" ", "") == "" then self.Library:Notify("Invalid theme name (empty)", 2) return end
            self:SaveCustomTheme(name)
            self.Library:Notify(string.format("Created theme %q", name))
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        ...
        presetsBox:AddButton("Load theme",      function() self:ApplyTheme(self.Library.Options.ThemeManager_CustomThemeList.Value) end)
        presetsBox:AddButton("Overwrite theme", function() self:SaveCustomTheme(self.Library.Options.ThemeManager_CustomThemeList.Value) end)
        presetsBox:AddButton("Delete theme",    function()
            local name = self.Library.Options.ThemeManager_CustomThemeList.Value
            local ok, err = self:Delete(name)
            if not ok then self.Library:Notify("Failed: " .. err) return end
            self.Library:Notify(string.format("Deleted theme %q", name))
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end)
        ...
        presetsBox:AddButton("Set as default",   function()
            local v = self.Library.Options.ThemeManager_CustomThemeList.Value
            if v and v ~= "" then
                self:SaveDefault(v)
                self.Library:Notify(string.format("Set default theme to %q", v))
            end
        end)
